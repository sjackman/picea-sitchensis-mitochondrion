...
# Load data
```{r load-data}
gff.all <- readGff3("psitchensismt_18.gff", isRightOpen = FALSE, quiet = TRUE)

gff <- gff.all[gff.all$type %in% c('mRNA', 'rRNA', 'tRNA'),]

scaffold_sizes <- read_tsv("psitchensismt_18.fa.fai",
...
data <- tibble(
	...
	mutate(
		Gene =  sub("[-_|].*", "", Name),
		Family.short = tolower(substr(Name, 1, 3))) %>%
	left_join(gene.types, by = "Family.short") %>%
...
data_scaffolds <- data %>%
	filter(Family.short != "orf") %>%
	mutate(Family = factor(Family.short, levels = gene.types$Family.short, labels = gene.types$Family_both))
...
ggplot(data_scaffolds) +
	...
	scale_fill_manual("", breaks = gene.types$Family_both, values = gene.types$Colour) +
...
```{r gene-order-and-size-large, fig.width = 8.5, fig.height = 11, dpi = 300}
...
write.table(data_grouped, 'psitchensismt_18.gff.family.tsv',
